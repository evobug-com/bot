...
export const execute = async ({ interaction, dbUser }: CommandContext): Promise<void> => {
	...
	try {
		const dailyCooldown = await orpc.users.stats.daily.cooldown({ userId: dbUser.id });
		...
		try {
			...
			const response = await orpc.users.stats.daily.claim({
				userId: dbUser.id,
				boostCount,
			});
		...
		const errorEmbed = createErrorEmbed(
			"Chyba",
			"Při vyzvedávání denní dávky došlo k chybě. Zkuste to prosím později.",
		);
		await interaction.editReply({ embeds: [errorEmbed] });
	}
};
